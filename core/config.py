...
DEFAULT_LOKI_URL = os.environ.get("LOKI_URL", "").rstrip("/")
LOKI_BEARER_TOKEN = os.environ.get("LOKI_BEARER_TOKEN", "")
LOKI_TIMEOUT_SEC = float(os.environ.get("LOKI_TIMEOUT_SEC", "15"))
...
def normalize_loki_environment(value: str) -> str:
    v = value.strip().lower().replace("-", "_").replace(" ", "_")
    if v in ("prod", "production"):
        return "prod"
    if v in ("dev", "develop", "development", "test", "testing", "qa", "devtest", "dev_and_test"):
        return "dev_test"
    return v


def load_loki_env_urls(logger: logging.Logger) -> Dict[str, str]:
    env_urls: Dict[str, str] = {}
    raw = os.environ.get("LOKI_ENV_URLS", "").strip()
    if not raw:
        logger.warning("LOKI_ENV_URLS is not set; Loki environment URLs are empty.")
        return env_urls

    try:
        parsed = json.loads(raw)
    except Exception:
        logger.warning("Failed to parse LOKI_ENV_URLS; ignoring.")
        return env_urls

    if not isinstance(parsed, dict):
        logger.warning("LOKI_ENV_URLS must be a JSON object; ignoring.")
        return env_urls

    for key, value in parsed.items():
        env_urls[normalize_loki_environment(str(key))] = str(value)
    return env_urls


LOKI_ENV_URLS = load_loki_env_urls(logging.getLogger(__name__))
